...
\@x{}\moduleLeftDash\@xx{ {\MODULE} GeneralDieHard}\moduleRightDash\@xx{}%
\begin{lcom}{0}%
...
In the movie Die Hard 3, the heros must obtain exactly 4 gallons of
 ...
 goal: to get \ensuremath{TLC} to solve the problem for us.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
\@x{ {\CONSTANT} BigJug ,\,\@s{9.82}}%
\@y{\@s{0}%
 The capacity of the bigger jug
}%
\@xx{}%
\@x{\@s{50.17} SmallJug ,\,}%
\@y{\@s{0}%
 The capacity of the smaller jug
}%
\@xx{}%
\@x{\@s{50.17} Goal\@s{25.70}}%
\@y{\@s{0}%
 The target amount of water
}%
\@xx{}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
We next declare the specification\mbox{'}s variables.
\end{cpar}%
\end{lcom}%
\@x{ {\VARIABLES} big ,\,\@s{8.87}}%
\@y{\@s{0}%
 The number of gallons of water in the 5 gallon jug.
}%
\@xx{}%
\@x{\@s{51.42} small\@s{4.09}}%
\@y{\@s{0}%
...
Now we define of the initial predicate, that specifies the initial
 values of the variables. I like to name this predicate \ensuremath{Init},
 but the
 name doesn\mbox{'}t matter.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Note: TLA+ uses the convention that a list of formulas bulleted by
 \ensuremath{\.{\land}
 } or \ensuremath{\.{\lor}} denotes the conjunction or disjunction of those
 formulas.
...
\end{cpar}%
\end{lcom}%
\@x{ FillSmall\@s{13.03} \.{\defeq} \.{\land} small \.{'} \.{=} SmallJug}%
\@x{\@s{72.70} \.{\land} big \.{'}\@s{10.88} \.{=} big}%
\@pvspace{8.0pt}%
\@x{ FillBig\@s{23.00} \.{\defeq} \.{\land} big \.{'}\@s{10.88} \.{=} BigJug}%
\@x{\@s{72.70} \.{\land} small \.{'} \.{=} small}%
\@pvspace{8.0pt}%
\@x{ EmptySmall \.{\defeq} \.{\land} small \.{'} \.{=} 0}%
\@x{\@s{72.70} \.{\land} big \.{'}\@s{10.88} \.{=} big}%
\@pvspace{8.0pt}%
\@x{ EmptyBig\@s{9.97} \.{\defeq} \.{\land} big \.{'}\@s{10.88} \.{=} 0}%
\@x{\@s{72.70} \.{\land} small \.{'} \.{=} small}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
We now consider pouring water from one jug into another. Again, since
 the jugs are not callibrated, when pouring from jug A to jug \ensuremath{B},
 it
 makes sense only to either fill \ensuremath{B} or empty A. And
 there\mbox{'}s no point in
 emptying A if this will cause \ensuremath{B} to overflow, since that could be
 accomplished by the two actions of first filling \ensuremath{B} and then
 emptying A.
 So, pouring water from A to \ensuremath{B} leaves \ensuremath{B} with the
 lesser of (i) the water
 contained in both jugs and (ii) the volume of \ensuremath{B}. To express this
 mathematically, we first define \ensuremath{Min(m,\,n)} to equal the minimum
 of the
 numbers \ensuremath{m} and \ensuremath{n}.
\end{cpar}%
\end{lcom}%
...
\end{cpar}%
\end{lcom}%
 \@x{ SmallToBig\@s{0.14} \.{\defeq} \.{\LET} poured \.{\defeq} Min ( big
 \.{+} small ,\, BigJug ) \.{-} big}%
\@x{\@s{95.16}}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{70.66} \.{\IN}\@s{4.09} \.{\land} big \.{'}\@s{10.88} \.{=} big \.{+}
 poured}%
\@x{\@s{95.16} \.{\land} small \.{'} \.{=} small \.{-} poured}%
\@pvspace{8.0pt}%
\@x{ BigToSmall \.{\defeq}}%
 \@x{\@s{8.2} \.{\LET} poured \.{\defeq} Min ( big \.{+} small ,\, SmallJug )
 \.{-} small}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 We define the next-state relation, which I like to call \ensuremath{Next}. A
 \ensuremath{Next
 } step is a step of one of the six actions defined above. Hence,
 \ensuremath{Next} is
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 We now define \ensuremath{TypeOK} to be the type invariant, asserting that
 the value
 of each variable is an element of the appropriate set. A type
 invariant like this is not part of the specification, but it\mbox{'}s
 generally a good idea to include it because it helps the reader
 understand the spec. Moreover, having \ensuremath{TLC} check that it is an
 invariant of the spec catches errors that, in a typed language, are
 caught by type checking.
\end{cpar}%
\end{lcom}%
\@x{ TypeOK \.{\defeq} \.{\land} small \.{\in} 0 \.{\dotdot} SmallJug}%
\@x{\@s{56.14} \.{\land} big\@s{10.88} \.{\in} 0 \.{\dotdot} BigJug}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
...
Remember that our heros must measure out 4 gallons of water.
 ...
 solved their problem when they reach a state with \ensuremath{big \.{=} 4}.
 We find a
 solution by having \ensuremath{TLC} check if big \# 4 is an invariant, which
 will
 ...
 \ensuremath{big \.{=} 4}. Such a behavior is the desired solution. (Because
 \ensuremath{TLC} uses a
