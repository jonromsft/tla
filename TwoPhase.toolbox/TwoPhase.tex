...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This specification describes the Two-Phase \ensuremath{Commit} protocol, in
 which a
 transaction manager (\ensuremath{TM}) coordinates the resource managers
 (\ensuremath{RMs}) to
 implement the Transaction \ensuremath{Commit} specification of module
 \ensuremath{TCommit}. In
 this specification, \ensuremath{RMs} spontaneously issue
 \ensuremath{Prepared} messages. We
 ignore the \ensuremath{Prepare} messages that the \ensuremath{TM} can send
 to the \ensuremath{RMs}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
In the protocol, processes communicate with one another by sending
 ...
 loss. (There\mbox{'}s no difference between a process not being able to
 ...
 a variable \ensuremath{msgs} whose value is the set of all messages that have
 been sent. Messages are never removed from \ensuremath{msgs}. An action that,
 in an implementation, would be enabled by the receipt of a certain
 message is here enabled by the existence of that message in
 \ensuremath{msgs}.
...
\end{lcom}%
 ...
 \@x{\@s{8.2} \.{\land} tmState\@s{0.89} \.{\in} \{\@w{init} ,\,\@w{committed}
 ,\,\@w{aborted} \}}%
...
\@x{\@s{8.2} \.{\land} tmState \.{'} \.{=}\@w{committed}}%
...
\end{lcom}%
\@x{\@s{8.2} \.{\land} tmState \.{=}\@w{init}}%
\@x{\@s{8.2} \.{\land} tmState \.{'} \.{=}\@w{aborted}}%
 \@x{\@s{8.2} \.{\land} msgs \.{'} \.{=} msgs \.{\cup} \{ [ type
 \.{\mapsto}\@w{Abort} ] \}}%
 \@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} rmState ,\, tmPrepared
 {\rangle}}%
...
We now assert that the Two-Phase \ensuremath{Commit} protocol implements the
 ...
 following statement
 defines \ensuremath{TC{\bang}TCSpec} to be formula \ensuremath{TCSpec} of
 module \ensuremath{TCommit}. (The TLA+
 \ensuremath{{\INSTANCE}} statement imports all the definitions from module
 \ensuremath{TCommit
} renamed in this way, thus avoiding any name conflicts that might exist
 between defined operators in the two modules. The constant \ensuremath{RM}
 and and
 variable \ensuremath{rmState} are the same in both modules.)
\end{cpar}%
\end{lcom}%
\@x{ TC \.{\defeq} {\INSTANCE} TCommit}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} TPSpec \.{\implies} TC {\bang} TCSpec}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
